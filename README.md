...
2. Computer random choice function (Math.random returns values between 0 and 1);

function getComputerChoice is:
	the computerChoice variable is a random number;
	if number > 0.7 returns "Scissors";
	else if number > 0.3 returns "Paper";
	else "Rock";

  show in console the message "Computer is choosing: " + the result from getComputerChoice;
